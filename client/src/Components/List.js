...
function Transaction({ category, handler }){
    ...
    return (
        <div className="item flex justify-center bg-gray-50 py-2 rounded-r" style={{ borderRight : `18px solid ${category.color ??  "#e5e5e5"}`}}>
            ...
            <span className='block w-full '>{category.name ?? ''} = Rs{category.amount ?? ''}</span>
        </div>
    )
}